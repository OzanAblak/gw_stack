...
3) Kullanıcı “satın al / devam et” butonuna tıklar.
   ...
   - Beklenen backend davranışı:
     - PAYMENT_ATTEMPT kaydı oluşturulur (`status=pending`).
...
- Son durumda:
  - SUBSCRIPTION `active`,
  - En az 1 PAYMENT_ATTEMPT `succeeded`,
...
7) Beklenen response:
   ...
   - `paymentAttempt.errorCode` dolu (örn. `CARD_DECLINED`)
...
- Senaryo 3 — Subscription görünürlüğü (aboneliği olmayan kullanıcı):
  - GET /api/billing/subscription
    - Varsayılan (parametresiz) çağrıda:
      - subscription.status = "active"  (örnek aktif abonelik)
  - GET /api/billing/subscription?testNoSubscription=true
    - HTTP status = 404
    - detail.code = "SUBSCRIPTION_NOT_FOUND"
    - detail.message = "Bu hesap için aktif bir abonelik bulunamadı."
